...
protected:
    ...
    const char * expression_parameter_argv[])
  {
    ...
    return rcl_content_filter_fallback_set(instance, &options);
  }
...
protected:
  void SetUp()
  {
    ...
    instance = rcl_content_filter_fallback_create(type_support);
    EXPECT_NE(instance, nullptr);
  }

  void TearDown()
  {
    rcl_content_filter_fallback_destroy(instance);
  }
...
TEST_F(TestCommonContentFilterAPI, is_enabled) {
  EXPECT_FALSE(rcl_content_filter_fallback_is_enabled(nullptr));
  EXPECT_FALSE(rcl_content_filter_fallback_is_enabled(instance));
  EXPECT_TRUE(
    set_options(filter_expression, expression_parameter.size(), expression_parameter.data()));
  EXPECT_TRUE(rcl_content_filter_fallback_is_enabled(instance));
}

TEST_F(TestCommonContentFilterAPI, evaluate) {
  EXPECT_FALSE(rcl_content_filter_fallback_evaluate(nullptr, nullptr, false));
  EXPECT_FALSE(rcl_content_filter_fallback_evaluate(instance, nullptr, false));
  EXPECT_FALSE(rcl_content_filter_fallback_evaluate(nullptr, nullptr, true));
  EXPECT_FALSE(rcl_content_filter_fallback_evaluate(instance, nullptr, true));
  ...
  EXPECT_TRUE(rcl_content_filter_fallback_evaluate(instance, &msg, false));
  EXPECT_TRUE(rcl_content_filter_fallback_evaluate(instance, &serialized_message, true));
  // after setting filter with "int32_value = 4"
  EXPECT_TRUE(
    set_options(filter_expression, expression_parameter.size(), expression_parameter.data()));
  // expect msg(int32_value = 3) return false
  EXPECT_FALSE(rcl_content_filter_fallback_evaluate(instance, &msg, false));
  EXPECT_FALSE(rcl_content_filter_fallback_evaluate(instance, &serialized_message, true));
  // update msg with 4
  msg.int32_value = 4;
  EXPECT_EQ(
    RMW_RET_OK,
...
TEST_F(TestCommonContentFilterAPI, set) {
  EXPECT_FALSE(rcl_content_filter_fallback_set(nullptr, nullptr));
  EXPECT_FALSE(rcl_content_filter_fallback_set(instance, nullptr));
  EXPECT_TRUE(
    set_options(filter_expression, expression_parameter.size(), expression_parameter.data()));
...
TEST_F(TestCommonContentFilterAPI, get) {
  EXPECT_FALSE(rcl_content_filter_fallback_get(nullptr, nullptr, nullptr));
  EXPECT_FALSE(rcl_content_filter_fallback_get(instance, nullptr, nullptr));
  EXPECT_TRUE(
    set_options(filter_expression, expression_parameter.size(), expression_parameter.data()));
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  EXPECT_FALSE(rcl_content_filter_fallback_get(instance, &allocator, nullptr));

  rmw_subscription_content_filter_options_t options =
    rmw_get_zero_initialized_content_filter_options();
  EXPECT_TRUE(rcl_content_filter_fallback_get(instance, &allocator, &options));
  OSRF_TESTING_TOOLS_CPP_SCOPE_EXIT(
  {
    rmw_subscription_content_filter_options_fini(&options, &allocator);
  });
...
TEST_F(TestComplexMsgCommonContentFilterAPI, set_and_evaluate) {
  ...
  for (auto & item : expectation) {
    ...
    EXPECT_EQ(
      item.evaluate_expectation,
      rcl_content_filter_fallback_evaluate(instance, &msg, false))
